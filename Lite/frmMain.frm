   ...
   Begin VB.Frame fraSteps 
      ...
      Begin VB.Frame fraOptions 
         ...
         Begin VB.Label lblCaption 
            Alignment       =   2  'Center
            AutoSize        =   -1  'True
            Caption         =   "Upgrade to Picture Scroller 2.0 Deluxe to have background music accompany the picture scrolling!"
            Height          =   720
            Index           =   4
            Left            =   60
            TabIndex        =   76
...
' file to the recent docs list; also run program.
...
If bMouseOver > 0 Then
    ...
    If bMouseOver = 3 Then
        ...
        If tPictureFiles.Count <= 4 Then
            ...
            iSize = (picShow.ScaleHeight - 30) / 2
...
ShellExecute GetDesktopWindow, "open", "https://secure.bmtmicro.com/ECommerce-OnSite/61600.html", "", "", 0

End Sub
Private Sub lblUpgrade_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
...
Select Case bTransition
    ...
    Case 3: LookUpTrans = "Circle In"
    Case 4: LookUpTrans = "Diagonal Slide Left Down"
    Case 5: LookUpTrans = "Diagonal Slide Right Down"
    Case 6: LookUpTrans = "Diagonal Slide Left Up"
    Case 7: LookUpTrans = "Diagonal Slide Right Up"
    Case 8: LookUpTrans = "Maze"
    Case 9: LookUpTrans = "Move Left"
    Case 10: LookUpTrans = "Random Dots (Small)"
End Select
...
If GetPicFromIndex(picPreview, lPictureIndex, picPreview.Width, picPreview.Height) = False Then
    With rPreview
        .Top = (picPreview.Height - (picPreview.TextHeight("A") * 2 + 10)) / 2
        .Right = picPreview.Width
        .Bottom = .Top + picPreview.TextHeight("A")
    End With

    picPreview.Picture = LoadPicture()
    DrawTextAPI picPreview.hdc, "Cannot", 6, rPreview, DT_CENTER Or DT_SINGLELINE Or DT_VCENTER Or DT_NOCLIP

    rPreview.Top = rPreview.Top + picPreview.TextHeight("A") + 10
    rPreview.Bottom = rPreview.Top + picPreview.TextHeight("A")

    DrawTextAPI picPreview.hdc, "Load Picture", 12, rPreview, DT_CENTER Or DT_NOCLIP
End If
...
If DirectSound.InitDirectSound(Me.hwnd) = True Then
    ' Load sound effects.
    DirectSound.OpenSound "WELCOME", False, 1, "WAV"
    DirectSound.OpenSound "SELECT", False, 2, "WAV"
    DirectSound.OpenSound "HELP", True, 3, "WAV"
End If
